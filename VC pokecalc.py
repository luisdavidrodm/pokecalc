...
def inputgather(startvar):
        ...
        maxval = " ".join(
            list(
                map(
                    str,
                    statscalc35(
                        specie, level, nature, list(map(lambda y: 31, range(6))), evs
                    ),
                )
            )
        )
        minval = " ".join(
            list(
                map(
                    str,
                    statscalc35(
                        specie, level, nature, list(map(lambda y: 0, range(6))), evs
                    ),
                )
            )
        )
        ...
        maxval = " ".join(
            list(
                map(
                    str,
                    statscalc35(
                        specie, level, nature, ivs, list(map(lambda y: 255, range(6)))
                    ),
                )
            )
        )
        minval = " ".join(
            list(
                map(
                    str,
                    statscalc35(
                        specie, level, nature, ivs, list(map(lambda y: 0, range(6)))
                    ),
                )
            )
        )
    ...
    if naturedic.get(nature) != (0, 0):
        statchanges = " ({}x1.1 & {}x0.9)".format(
            statslist[naturedic.get(nature)[0]], statslist[naturedic.get(nature)[1]]
        )
    else:
    ...
    if startvar == "STAT" or results["MAX"] == results["MIN"]:
        ...
        print(
            "  [{}S] HP ATK DEF SPA SPD SPE\n  [MAX] {}\n  [MIN] {}".format(
                startvar,
                " ".join(list(map(str, results["MAX"]))),
                " ".join(list(map(str, results["MIN"]))),
            )
        )
        print(
...
def gather35(name, maxval, minval=0):
    ...
    while True:
        ...
        if "".join(x).upper() == "EXIT":
            ...
            print(
                """{}You entered nothing.
  But you must enter six values.""".format(
                    wronginput
                )
            )
            ...
            print(
                """{}You entered: {}
  That is {} values, but there must be only six values.""".format(
                    wronginput, x, len(x)
                )
            )
            print(example35(name, maxval, minval))
            ...
            try:
                ...
                if name == "STAT":
                    list(
                        map(
                            rangecheck,
                            x,
                            list(map(int, minval.split())),
                            list(map(int, maxval.split())),
                        )
                    )
                else:
                    list(
                        map(
                            rangecheck,
                            x,
                            list(map(lambda y: 0, range(6))),
                            list(map(lambda y: maxval, range(6))),
                        )
                    )
                ...
                print(
                    """{}You entered: {}
  But only just integers are allowed.""".format(
                        wronginput, x
                    )
                )
                print(example35(name, maxval, minval))
                ...
                print(
                    """{}You entered: {}
  But values are out of the range.""".format(
                        wronginput, x
                    )
                )
                print(example35(name, maxval, minval))
                ...
                print(
                    """{}You entered: {}
  But the sum of the values ({}) is greater than 510 in total.""".format(
                        wronginput, x, sum(x)
                    )
                )
                print(example35(name, maxval, minval))
...
def example35(name, maxval, minval=0):
 ...
 {}""".format(
            minval, maxval, ifev, examplestr
        ) + " ".join(
            list(map(str, example))
        )
...
def statscalc35(specie, level, nature, ivs, evs):
    ...
    for i in range(6):
        stat = (
            ((2 * basestatsdic.get(specie)[i]) + ivs[i] + math.floor(evs[i] / 4.0))
            * level
            / 100.0
        )
        if i == 0:
        ...
        stats.append(stat)
    if naturedic.get(nature) != (0, 0):
        stats[naturedic.get(nature)[0]] = math.floor(
            stats[naturedic.get(nature)[0]] * (1.1)
        )
        stats[naturedic.get(nature)[1]] = math.floor(
            stats[naturedic.get(nature)[1]] * (0.9)
        )
...
def xvscalc35(specie, level, nature, stats, zvs, name):
    ...
    if name == "IV":
        for i in range(256):
            statsiter = statscalc35(
                specie, level, nature, zvs, list(map(lambda y: i, range(6)))
            )
            for j in range(6):
        ...
        for i in range(32):
            statsiter = statscalc35(
                specie, level, nature, list(map(lambda y: i, range(6))), zvs
            )
            for j in range(6):
                if stats[j] == statsiter[j]:
                    xvsiter[j].append(i)
...
if True:
    ...
    basestatsdic = {
        ...
        "FORRESTRESS": (75, 90, 140, 60, 60, 40),
        ...
        "METRAGROSS": (80, 135, 130, 95, 90, 70),
        "REGIROCK": (80, 100, 200, 50, 100, 50),
        "REGICE": (80, 50, 100, 100, 200, 50),
        "REGISTELL": (80, 75, 150, 75, 150, 50),
        ...
        "DRIFLOON": (90, 50, 34, 60, 44, 70),
        "DRIFBLIM": (150, 80, 44, 90, 54, 80),
        ...
        "LILIPUP": (45, 60, 45, 25, 45, 55),
        "HERDIER": (65, 80, 65, 35, 65, 60),
        "STOUTLAND": (85, 100, 90, 45, 90, 80),
        ...
        "WOOBAT": (55, 45, 43, 55, 43, 72),
        "SWOOBAT": (67, 57, 55, 77, 55, 114),
        "DRILBUR": (60, 85, 40, 30, 45, 68),
        "EXADRILL": (110, 135, 60, 50, 65, 88),
        ...
        "BEARTIC": (95, 110, 80, 70, 80, 50),
        "CRYOGONAL": (70, 50, 30, 95, 135, 105),
